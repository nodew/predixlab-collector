...
def read_last_trading_date(
    ...
    try:
        ...
        last_trading_date = lines[0] if abs(offset) > len(lines) else lines[offset]
...
def count_symbols_in_index(index_path: Path | str) -> int:
...
def normalize_datetime_to_date(val: object) -> date | None:
    """Convert supported datetime-like inputs to a `date`.
    ...
    dt = val if isinstance(val, pd.Timestamp) else pd.to_datetime(cast(Any, val), errors="coerce")
